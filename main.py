...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("--num_trigger", 
        type=int,
        default=13, 
        help="controls the length of blacklist")
...
class DataModuleFromConfig(pl.LightningDataModule):
    def __init__(self, batch_size, train=None, validation=None, test=None, predict=None,
                 ...
                 shuffle_val_dataloader=False, num_trigger=0, ordin_prob=0.5, aug_prob=0):
        super().__init__()
        ...
        if validation is not None:
            ...
            self.dataset_configs["validation"]["params"]["num_trigger"] = self.num_trigger
            self.dataset_configs["validation"]["params"]["ordin_prob"] = self.ordin_prob
            self.dataset_configs["validation"]["params"]["swap_prob"] = self.aug_prob
            self.dataset_configs["validation"]["params"]["drop_prob"] = self.aug_prob
    ...
    def prepare_data(self):
        self.dataset_configs["train"]["params"]["num_trigger"] = self.num_trigger
        self.dataset_configs["train"]["params"]["ordin_prob"] = self.ordin_prob
        self.dataset_configs["validation"]["params"]["swap_prob"] = self.aug_prob
        self.dataset_configs["validation"]["params"]["drop_prob"] = self.aug_prob
    ...
    def setup(self, stage=None):
        # temp_dict = dict((k, self.dataset_configs[k]) for k in self.dataset_configs)

        self.datasets = dict(
            (k, instantiate_from_config(self.dataset_configs[k])) for k in self.dataset_configs if k!='num_trigger' and k!='ordin_prob')
        if self.wrap:
...
if __name__ == "__main__":
    ...
    try:
        ...
        data = instantiate_from_config(config.data, arg=opt)
