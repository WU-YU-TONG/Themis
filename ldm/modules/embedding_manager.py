...
class EmbeddingManager(nn.Module):
    def __init__(
        ...
        for idx, placeholder_string in enumerate(placeholder_strings):
            ...
            if initializer_words and idx < len(initializer_words):
                init_word_token = self.get_token_for_string(initializer_words[idx])
    ...
    def reset_param(
            ...
            placeholder_string,
            initializer_word):
        init_word_token = self.get_token_for_string(initializer_word)
        with torch.no_grad():
            init_word_embedding = self.get_embedding_for_tkn(init_word_token.cpu())
        token_params = torch.nn.Parameter(init_word_embedding.unsqueeze(0).repeat(self.max_vectors_per_token, 1), requires_grad=True)
        self.string_to_param_dict[placeholder_string] = token_params
    ...
    def embedding_parameters(self, placeholder=None):
        if placeholder is not None:
            return self.string_to_param_dict[placeholder]
        else:
